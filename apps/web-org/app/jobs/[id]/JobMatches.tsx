...
export function JobMatches({ jobId }: { jobId: string }) {
  ...
  if (error) return <div style={{ marginTop: 'var(--cl-s7)' }}><ErrorNotice code={error.code} details={error.details} /></div>;
  if (!run) return null;
          ...
          {run.matched.map((m) => (
            <div key={m.candidateId}>
              <div style={{ display: 'flex', gap: 'var(--cl-s3)', alignItems: 'baseline', marginBottom: 'var(--cl-s2)' }}>
                <Link href={`/candidates/${m.candidateId}`} style={{ fontFamily: 'var(--cl-font-mono)', fontWeight: 600 }}>
                  {m.displayCode}
                </Link>
                {m.candidateName
