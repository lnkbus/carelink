  ...
  return (
    <OrgShell orgName={org.name} verificationStatus={org.verificationStatus}>
      <PageHeader
        ...
        actions={<Link href="/org/jobs/new"><span style={{ fontSize: 'var(--cl-caption)' }}>채용 요청 등록 →</span></Link>}
      />
      <div className="cl-body">
        ...
        <Section
          ...
          aside={<Link href="/org/jobs" style={{ fontSize: 'var(--cl-caption)' }}>전체 보기 →</Link>}
        >
          <DataTable
            columns={[
              { key: 'title', label: '요청' },
          ...
          >
            {jobs.items.slice(0, 7).map((j) => (
              <Tr key={j.id}>
                <Td>
                  <Link href={`/org/jobs/${j.id}`}>{j.title ?? '(제목 없음)'}</Link>
                </Td>
        ...
        <Section
          ...
          aside={<Link href="/org/interviews" style={{ fontSize: 'var(--cl-caption)' }}>면접 관리 →</Link>}
        >
          <DataTable
            columns={[
