...
apps/web/            DESK 웹 한 벌 — Next.js  (:3100)
  └─ app/
       ...
       signup/         기관 · 파트너 가입 신청 (후보자·간병사·보호자는 앱에서)
       pending/        승인 대기 — 무엇을 신청했고 누가 승인하는가
       admin/          Admin Console      (V1)  · approvals/ 운영자 승인 큐
       org/            Organization Web   (V1)  · members/   기관 관리자 승인 큐
       no-access/      역할도 대기 중인 신청도 없는 계정 안내
...
**주의 — `org`는 역할이 아니라 관계입니다.** 검증 완료 + 후보자의 면접 수락을
모두 통과한 뒤에만 붙습니다. 기관 담당자가 평소 들고 있는 것은
`org_masked`입니다. 후보자 개인정보가 아닌 DTO(예: 승인 대기 목록)에 `org`를
적으면 **기관 담당자에게 빈 화면이 나갑니다** — 실제로 그렇게 나왔습니다.

### 5.2-1 소속 승인과 기관 검증은 다른 게이트입니다

헷갈리기 쉬워 여기 적어 둡니다. 둘을 한 값으로 합치려는 유혹이 반복됩니다.

| | 무엇을 묻나 | 어디에 | 누가 |
|---|---|---|---|
| **소속 승인** | 이 사람이 이 기관 사람이 맞나 | `user_roles.approved_at` | 운영자(첫 담당자) · 그 기관의 `ORG_ADMIN`(이후) |
| **기관 검증** | 이 기관이 실재하는 사업자가 맞나 | `organizations.verification_status` | 운영자 |

소속 승인을 받아도 검증 전에는 후보자가 `CD-1001`로만 보입니다(§6-6).
반대로 검증된 기관이라도 승인받지 못한 사람은 아무 화면도 열지 못합니다.

**승인이 필요한 역할 목록은 `needsApproval()` 한 곳입니다.** 종전에 같은
목록이 둘 있었고, 한쪽에 `PARTNER`를 넣지 않아 승인 대기 중인 파트너가
권한을 그대로 받았습니다. 목록을 늘릴 때 두 곳을 기억해야 하는 구조는
반드시 한쪽을 잊습니다.

**후보자·간병사·보호자에게 승인을 걸지 마세요.** 본인이 고르면 그 자리에서
열립니다. 승인을 걸면 앱을 깔고 번호를 넣은 사람이 아무것도 못 하는 화면에서
...
**알림 문구는 예외입니다 — 서버가 붙입니다.** 오류 코드와 헷갈리지 마세요.
오류는 클라이언트가 번역하지만, 알림은 같은 내용이 PUSH·SMS·알림톡으로
나갈 때 길이가 달라야 하고 운영 중에 배포 없이 고쳐야 합니다. 그래서
표에서 옵니다.

**코드를 추가하면 4개 언어를 전부 넣으세요.** 하나만 빠뜨리면 그 언어
사용자에게 한국어가 나가고, 도달은 하므로 아무도 오류로 보고하지 않습니다.
`notification.template.spec.ts`가 조합을 검사합니다.

**`enqueue`만 하고 끝내지 마세요.** 종전에 호출이 11군데였는데 읽는
경로가 없어 전부 묻혀 있었습니다. 사용자가 그 알림을 어디서 보는지까지
확인하세요.
...
## 10-1. 디자인 시안을 반영하는 절차

시안이 오면 **파일을 받지 말고 직접 읽습니다.**

### 시안 쪽에서 할 일

1. Claude Design 캔버스를 **게시**합니다 (게시해야 코드에서 읽힙니다).
2. 아트보드 제목에 **화면 ID**를 답니다 — `SCR-001 v2`처럼.

**새 번호 체계를 만들지 마세요.** `wireframe/index.html`의 `SCREENS` 배열이
이미 36개 화면에 `SCR-###`를 부여했고 그것이 사양의 단일 출처입니다(§0).
이미지 번호를 따로 매기면 지도가 둘이 되고, 둘은 반드시 어긋납니다.
인트로처럼 `SCR-###`가 없는 화면은 라우트를 그대로 씁니다 — `/(intro)`.

### 코드 쪽에서 할 일

```
1. 아티팩트 목록에서 캔버스를 찾는다        Artifact action:"list"
2. 전체 HTML을 읽는다                       Artifact action:"read" url:…
3. 구조를 기계로 대조한다                   node apps/web/scripts/design-diff.mjs <URL>
4. 차이를 판단해 반영한다                   사람이 판단, 기계는 목록만
5. 실제 브라우저로 확인한다                 앵커·가로 넘침·4개 언어
```

**3번을 건너뛰지 마세요.** 스크린샷을 나란히 놓고 보면 놓칩니다 — 실제로
놓쳤습니다. 시안이 '다섯 단계' 섹션을 걷어냈는데 실물에 남아 같은 이야기를
두 번 했고, '산업' 섹션은 내비에 없어 스크롤로만 닿았습니다. 둘 다 눈으로는
안 보이고 섹션 id 목록으로는 한눈에 보입니다.

### 시안을 따르지 않아도 되는 것

정적 시안은 **가입 경로도 다국어도 모릅니다.** `design-diff`가 내는 차이는
오류가 아니라 목록입니다. 아래는 시안에 없어도 유지합니다:

- 가입·승인 동선 (`/signup`, `/pending`) — 시안에는 '로그인'만 있습니다
- 4개 언어 전환기 — 시안은 `KO` 버튼 하나입니다
- 깨진 상대경로 이미지 (`./screenshot-….png`) — 옮기지 않습니다

### 빠르게 바꾸는 순서

바꾸려는 것이 무엇이냐에 따라 손대는 곳이 다릅니다. **위로 갈수록 빠릅니다.**

| 바꾸는 것 | 어디 | 비용 |
|---|---|---|
| 색·간격·radius·글꼴 | `packages/ui/src/tokens/tokens.css` | 한 파일. 웹·앱이 같이 바뀝니다 |
| 문구 | `lib/intro-i18n.ts` | 4개 언어를 **함께** 넣습니다 (테스트가 강제) |
| 항목 추가·삭제 (산업·단계·카드) | `lib/intro.ts` 배열 | 한 줄 + 문구 키 |
| 섹션 순서·구조 | `app/page.tsx` + `globals.css` | 여기서부터 느립니다 |

인트로 CSS는 토큰 92 · 하드코딩 14로 대부분 토큰입니다. `page.tsx`에
인라인 스타일은 0개입니다 — **시안의 인라인 스타일을 그대로 옮기지
마세요.** 그 순간 토큰이 무력해지고 다음 변경이 전부 수작업이 됩니다.
